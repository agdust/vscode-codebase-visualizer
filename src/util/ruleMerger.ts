...
export class RuleMerger {
	...
	constructor(
		...
		virtualProps: Record<string, any> = {}
	) {
	...
	merge<T>(items: T[]): Record<string, any>[] {
		return (
			_(items)
				...
				.map((group) => {
					const mergedObj = _(this.rules).reduce<Record<string, any>>(
						(accum, { prop, accessor, rule, virtual }) => {
...
export type MergeRule<Name extends string = string> =
	| { rule: Name; [key: string]: any }
	| string;
