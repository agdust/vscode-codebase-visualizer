	...
	if (!installed) {
		await vscode.window.withProgress(
			...
			async (progress, token) => {
				try {
					await child_process_promise.spawn(
						pythonPath,
						["-m", "pip", "install", "pydeps"],
	...
	const settings: VisualizationSettings = {
		...
		connectionDefaults: {
			tooltip: (conn, vis) =>
				_(conn.connections)
					.map(
						(c) => `"${vis.getRelativePath(c.from)}" -> "${vis.getRelativePath(c.to)}"`
					)
					.sortBy()
					.join("<br/>"),
		},
...
export async function getDependencyGraph(
	...
	const promises = _(fsPaths)
		.map<Promise<[string, string[]]>>(async (fsPath) => {
			...
			if (ext == ".py" && !(await isSymlink(Uri.file(fsPath)))) {
				const result = await child_process_promise.spawn(
					pythonPath,
				...
				const realPaths = await Promise.all(
					_(graph)
						.map<Promise<[string, any]>>(async (info, moduleName: string) => [
							moduleName,
							typeof info.path == "string" ? await fsp.realpath(info.path) : info.path,
						])
						.value()
				);
				...
				const modules: string[] =
					_(graph).find((info) => info.path == fsPath)?.imports ?? [];
	...
	return (await Promise.all(promises)).flatMap(([source, dependencies]) =>
