...
export async function activate(context: vscode.ExtensionContext) {
	...
	context.subscriptions.push(
		...
		vscode.commands.registerCommand("stackTraceVisualization.start", async () => {
			...
			stackTraceVisualization.setVisualization(
				await createStackTraceVisualization(cbrvAPI)
			);
...
async function createStackTraceVisualization(cbrvAPI: API): Promise<Visualization> {
	const settings: VisualizationSettings = {
		...
		connectionDefaults: {
			tooltip: (conn, vis) => {
				const tooltips = conn.connections.map(
					(c) =>
						`to Thread ${c.threadId} "${c.toName}" "${vis.getRelativePath(
							c.to
						)}:${vis.getLine(c.to)}"`
				);
				return _(tooltips)
...
class StackTraceVisualization implements vscode.DebugAdapterTrackerFactory {
			...
			onDidSendMessage: async (msg: DebugProtocol.ProtocolMessage) => {
				if (msg.type == "event" && (msg as DebugProtocol.Event).event == "stopped") {
					...
					const pairs = await Promise.all(
						[...this.stackTraces.keys()].map<
							Promise<[number, DebugProtocol.StackTraceResponse["body"]]>
						>(async (threadId) => {
					...
					this.stackTraces = new Map(
						pairs
							...
							.map(([threadId, frame]) => {
								const simpleFrames = [...frame.stackFrames].reverse().map((frame) => ({
	...
	async updateCallback(visState: VisualizationState) {
		visState.connections = [...this.stackTraces.entries()].flatMap(
			([threadId, frames], threadIndex) =>
				frames
					.filter((frame) =>
						visState.files.some((uri) => frame.file.fsPath == uri.fsPath)
					)
					.map((frame, i, arr) => ({
						from:
							i == 0
