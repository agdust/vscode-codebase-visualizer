Manual acceptance tests for the graphical aspects of Repovis which can't easily be tested automatically.

# Sample repositories

Several sample repositories are in the [sample-repos](./sample-repos) directory. They are used in both the unit
tests as well for manual acceptance testing.

## [minimal](./sample-repos/minimal)
...
- Use the context menu on `A/E.txt` to "Copy Path"
  - Should copy full path `.../sample-repos/minimal/A/E.txt` to clipboard
...
- Use the context menu on `A` to "Copy Path"
  - Should copy full path `.../sample-repos/minimal/A` to clipboard
