...
describe("Test geometry.ts", () => {
	...
	it("extendLine", () => {
		...
		).to.be.deepCloseTo([6.536, 7.536], 0.0005);
	...
	it("closestPointOnBorder", () => {
		...
		expect(() => geo.closestPointOnBorder([0, 0], border)).to.throw(
			"Point [0,0] is outside border [1,2,5,6]",
		);
	});
	...
	it("moveAlongBorder", () => {
		...
		expect(() => geo.moveAlongBorder([0, 0], 1, border)).to.throw(
			"Point [0,0] is not on border [1,2,5,6]",
		);
		expect(() => geo.moveAlongBorder([10, 10], 1, border)).to.throw(
			"Point [10,10] is not on border [1,2,5,6]",
		);
		expect(() => geo.moveAlongBorder([1, 10], 1, border)).to.throw(
			"Point [1,10] is not on border [1,2,5,6]",
		);
	});
	...
	it("polarToRect", () => {
		...
		expect(geo.polarToRect(Math.PI / 4, 1)).to.be.deepCloseTo([
			Math.SQRT2 / 2,
			Math.SQRT2 / 2,
		]);
	...
	it("snapAngle", () => {
		...
		// offset
		expect(geo.snapAngle(0.01, Math.PI / 4, Math.PI / 8)).to.be.closeTo(
			Math.PI / 8,
			1e-8,
		);
		expect(geo.snapAngle(2.01 * Math.PI, Math.PI / 4, Math.PI / 8)).to.be.closeTo(
			Math.PI / 8,
			1e-8,
		);
	});
	...
	it("unitVector", () => {
		...
		expect(geo.unitVector([5, 0, 5])).to.be.deepCloseTo([
			Math.SQRT2 / 2,
			0,
			Math.SQRT2 / 2,
		]);
